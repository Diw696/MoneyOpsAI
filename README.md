...
**A human-in-the-loop financial incident investigation platform that detects statistically meaningful anomalies across payment, refund, and webhook activity, retrieves historical cases from PostgreSQL-backed case memory, uses Gemini to investigate evidence, and proposes governed remediation actions for human approval.**

It investigates payment failures the way a human financial analyst would — then asks permission before doing anything about it.
...
- The bulk of the transaction volume used to demonstrate detection at scale comes from "Incident Lab" — a synthetic financial event stream generated to reproduce realistic operational conditions that are difficult to safely produce in Razorpay Test Mode, not real Razorpay traffic. This exists because safely reproducing a realistic, large-scale gateway failure (hundreds of correlated failed payments in a tight time window) isn't something you can responsibly do against a real payment system, even in test mode. Every simulated record is tagged `incident_lab` end-to-end, in the database, the API, and the UI — it is never blended into a number that implies it's real.
- Incident Lab behaves as a continuous simulated production feed, not a one-shot dataset: each "Generate New Simulation" appends a new, internally-coherent batch of events (new seed, new timestamps, new IDs) on top of everything generated before it — nothing is deleted or overwritten. Most batches are ordinary background noise with no anomaly at all; the rest inject exactly one coherent incident scenario, and a small minority inject two independent scenarios together (a compound incident). The generator supports gateway failure spikes, merchant refund spikes, duplicate-refund/retry-race activity, and webhook delivery failures — each backed by real, internally-consistent PostgreSQL records (concentrated error codes, a specific merchant's own refund history, duplicated refund rows on the same payment), not just a different incident title on the same shape of data.
...
For a technical reader who wants the real architecture: simulated production events and real Razorpay Test Mode data both flow through a canonical ingestion pipeline into PostgreSQL → an unsupervised **IsolationForest** model plus statistically-grounded per-merchant rules (a Wilson-score confidence floor, not just a raw sample-size cutoff) score gateway- and merchant-level features for anomalies → a new incident is created, in a `not_investigated` state, entirely separate from whether anyone has asked Gemini to look at it yet → a human explicitly triggers a **Gemini** investigation, which uses real multi-turn tool-calling against the database (not a single canned prompt) and is restricted to only the tools that make sense for that incident's entity type → PostgreSQL-backed **historical case retrieval** (case-based retrieval from prior investigations, not model retraining) surfaces relevant precedent → Gemini returns an evidence-grounded recommendation → a three-tier Action Governor enforces human approval before executing a safe, logged simulation → the incident closes and its investigation becomes case memory for the next one → the cycle repeats as new simulated events arrive.
...
Everything described above is fully working and has been re-verified end to end, including a full regression test suite (54/54 passing). Deliberately out of scope for this build: at real production scale, ingestion would use something like Kafka or Flink instead of the current direct pipeline, and case-memory retrieval would use a dedicated vector database instead of in-process similarity search — both were conscious scope decisions for a focused submission, not oversights.
